	...
	function run() {
		describe( "My RESTFUl Service", function() {
			...
			it("can display the version", function () {
				var event = this.request("/api/v1/echo/version");
				var response = event.getPrivateValue("response");
				expect(response.getError()).toBeFalse( "Version Endpoint should not return an error" );
				expect(response.getData()).toBe("Version 1");
			  });
